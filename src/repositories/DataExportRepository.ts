...
export const DataExportRepository = {
  async buildExport(babyId: string): Promise<ExportPayload> {
    ...
    const [profileResult, membersResult, careLogs, growthRecords, diaryEntries, diaryMedia,
      ...
      sb.from("profiles").select("id,display_name,nickname,avatar_url,avatar_storage_path,default_relation,residence_country,preferred_language,guardian_birth_date,created_at,updated_at").eq("id", user.id).maybeSingle(),
