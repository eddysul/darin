...
export type TrialTimeOption = {
  id: string;
  labelEn: string;
  labelKo: string;
};

export const TRIAL_TIMES: TrialTimeOption[] = [
...
/** Demo anchor — matches prototype timeline (June 20, 2026) */
...
export function formatTrialLabels(date: Date, time: TrialTimeOption): { en: string; ko: string } {
...
export function buildTrialSlot(date: Date, time: TrialTimeOption): TrialSlot {
  const labels = formatTrialLabels(date, time);
...
export function parseTrialSlotId(slotId: string): { date: Date | null; timeId: string | null } {
