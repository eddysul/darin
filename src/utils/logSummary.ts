...
export function formatTimelineLabel(entry: BabyLogEntry, customCategories: CustomCategory[] = []): string {
  ...
  if (!isCustomCategoryKey(entry.cat)) {
    ...
    if (entry.cat === "storedMilk") return entry.amount ? `저장 모유 ${formatVolume(entry.amount)}` : "저장 모유 수유";
    ...
    if (entry.cat === "pump" && entry.amount) return `유축 ${formatVolume(entry.amount)}`;
    if (entry.cat === "sleep") {
