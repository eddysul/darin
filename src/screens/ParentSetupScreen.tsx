...
export function ParentSetupScreen({ onComplete }: Props) {
  const { t } = useLanguage();
  const [draft, setDraft] = useState<UserProfile>({ ...DEFAULT_PARENT_PROFILE, name: "", location: "" });
  ...
  const handleContinue = () => {
    ...
    onComplete({
      ...
      location: draft.location.trim() || DEFAULT_PARENT_PROFILE.location,
    });
  };
    ...
    <ScreenBackground style={{ flex: 1 }}>
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <Text style={styles.title}>{t("parentSetup.title")}</Text>
          <Text style={styles.subtitle}>{t("parentSetup.subtitle")}</Text>
          ...
          >
            <Text style={styles.ctaText}>{t("parentSetup.cta")}</Text>
          </PressSlide>
        </ScrollView>
      </KeyboardAvoidingView>
    </ScreenBackground>
...
const styles = StyleSheet.create({
  ...
  toggleBtnActive: { borderColor: colors.yellow, backgroundColor: colors.yellowSoft },
