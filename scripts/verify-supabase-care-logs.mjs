...
import { cleanupQaAccounts, createQaAccount } from "./lib/qa-auth.mjs";
...
const qaAccount = await createQaAccount("care-logs");
const client = qaAccount.sb;
...
if (!schemaOk) {
  ...
  await cleanupQaAccounts([qaAccount]);
  finish(2);
}

// --- 2) Confirmed email auth ---
const userId = qaAccount.user.id;
pass(`Confirmed-email QA auth ok (user ${userId.slice(0, 8)}…)`);
...
const stranger = createClient(url, publicKey, {
