-- B0.4a P1-1: a Darin ID request cannot outlive the issuer's authority.
--
-- The request row is historical context only.  On acceptance, the issuer's
-- current membership for the exact baby is locked and revalidated before any
-- membership/friend grant or notification side effect is attempted.
...
begin
  ...
  -- Lock the pending request first.  This binds the response to the intended
  -- receiver and serializes replay/response races for this request.
  ...
  for update;
  if not found then
    raise exception 'invite request unavailable' using errcode = 'P0002';
  end if;
  ...
  if v_request.request_type is null
     or v_request.request_type not in ('family', 'friend')
     or v_request.permission_role is null
     or v_request.relationship_label is null then
    raise exception 'invite request invalid' using errcode = '22023';
  end if;

  v_response_status := case when p_accept then 'accepted' else 'declined' end;
  if p_accept then
    -- A requester's historical role, requested role, and sender id are not an
    -- authorization grant.  The sender must still be an active admin for the
    -- exact baby at response time.  Explicit NULL-safe predicates ensure a
    -- malformed historical row fails closed rather than passing SQL UNKNOWN.
    if v_request.sender_id is null or v_request.baby_id is null then
      raise exception 'invite issuer no longer has permission' using errcode = '42501';
    end if;

    perform 1
    from public.baby_members as issuer
    where issuer.baby_id = v_request.baby_id
      and issuer.user_id = v_request.sender_id
      and issuer.status::text is not distinct from 'active'
      and issuer.permission_role::text is not distinct from 'admin'
    for update;
    if not found then
      raise exception 'invite issuer no longer has permission' using errcode = '42501';
    end if;
    ...
    -- Declining does not create a membership, so it remains possible after
    -- the issuer is removed or demoted; only acceptance needs revalidation.
    insert into public.notification_events (
      recipient_id, actor_id, baby_id, event_type, title, body, data, dedupe_key, status
    ) values (
      v_request.sender_id,
