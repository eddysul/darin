-- Final B0.4c legacy recovery: pre-cutover rows never had an installation
-- secret, so a disabled proofless Expo token otherwise becomes impossible to
-- re-register. Treat possession of that high-entropy provider token by an
-- authenticated app session as a one-time bootstrap only for inert legacy
-- rows. Active rows and every proof-bound row remain ownership-protected.
...
begin
  ...
  -- Consume only the exact provider token from inert proofless legacy state.
  -- Device-ID-only legacy rows may remain for audit/history and cannot block.
  delete from public.push_tokens
  where user_id <> v_user_id
    and expo_push_token = p_expo_push_token
    and disabled_at is not null
    and installation_secret_hash is null;
