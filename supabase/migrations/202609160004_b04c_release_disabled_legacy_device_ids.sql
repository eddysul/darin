-- Final B0.4c rollout repair: disabled, proofless legacy rows must not reserve a
-- local device ID forever. Device IDs are installation-local and are only
-- unique within a user account. Keep Expo token ownership protected across
-- accounts, and keep active or proof-bound device rows protected.
...
begin
  ...
  -- An Expo token is provider-issued and remains ownership-sensitive even if
  -- its legacy row is disabled. A device ID alone is not globally unique: an
  -- inert pre-cutover row without installation proof may no longer block a
  -- new authenticated account from registering a different Expo token.
  perform 1
  ...
  where user_id <> v_user_id
    and (
      ...
      or (
        ...
        and not (disabled_at is not null and installation_secret_hash is null)
      )
    )
  for update;
  ...
  where user_id <> v_user_id
    ...
    and (expo_push_token = p_expo_push_token or device_id = p_device_id);
